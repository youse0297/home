...
BRDF、光照合成和调试输出已由 `Assets/_TA/Shaders/Library/TA_ShaderLibrary.hlsl` 统一提供，BasePass 只保留贴图采样、TBN 法线转换和 URP 光照数据适配。模块依赖与公共接口见 [Unity HLSL 源码库骨架](UNITY_HLSL_SOURCE_LIBRARY.md)。
