...
- `Assets/_TA/Shaders/Library/TA_ShaderLibrary.hlsl` 是 Renderer 侧唯一聚合入口，按依赖顺序装配 11 个模块；`TA_VertexDeformation.hlsl` 位于低层位移/动画与 PBR 输入之间。
...
- `Assets/_TA/ShaderGraph/Library` 继续服务 Shader Graph 节点，不与 Renderer 源码库互相包含。运行 `Tools/ValidateHlslSourceLibrary.ps1` 检查 38 个公共符号；高层位移结构和单入口由 `Tools/ValidateVertexDisplacementModularization.ps1` 验证。完整约定见 `../docs/UNITY_HLSL_SOURCE_LIBRARY.md` 和 `../docs/UNITY_VERTEX_DISPLACEMENT_MODULARIZATION.md`。
...
- `TA_VertexAnimation.hlsl` 提供显式时间的行进正弦、高度锚定权重，以及法线波浪与风向偏移的加法组合。
- BasePass 使用 `_Time.y` 驱动，`_WaveAmplitude` 和 `_WindAmplitude` 默认均为 `0`；`Tools/ValidateWaveWindAnimation.ps1` 固定 15 组数值与源码接线检查。
- 当前动画采用对象空间相位且只接入 `UniversalForward`；跨对象连续波场、动画法线重建和 Shadow/Depth 同步属于后续边界。

## 顶点位移模块化

- `TA_VertexDeformationInput`、`TA_VertexDeformationConfig` 和 `TA_VertexDeformationResult` 固定消费端与公式层的数据边界。
- `TA_EvaluateVertexDeformationOS` 统一执行高度位移、波浪与风摆；BasePass 只采样高度、绑定参数并消费最终 `positionOS`。
- `Tools/ValidateVertexDisplacementModularization.ps1` 固定 6 组组合结果，并检查依赖方向、无隐藏资源/时间全局和 BasePass 不再调用低层函数。
...
本阶段已建立工程、资产基线、BaseColor/Normal/ORM 输入契约、三个 URP Lit 材质实例、参数边界矩阵、可生成的 Shader Graph Custom Function 子图示例、材质函数库 v1、Standalone 贴图压缩策略与对照基准、三档 LOD 基础策略和切换场景、RenderDoc 截帧准备层、URP Forward BasePass 光照拆解视图，以及包含结构化顶点变形和简化 PBR 输入层的 Renderer HLSL 源码库。完整 Shader Graph 主材质属于后续日程任务。
...
The renderer-facing HLSL source library is v1.8.0. BasePass consumes vertex deformation through one structured entry point; low-level height, wave and wind modules remain independently validated and default to zero amplitude.
