...
| #59 | Renderer BasePass 缺少可复用的高度图顶点位移、明确空间顺序和专项数值回归 | 2026-08-26 | 已新增中心化高度解码、对象空间法线位移、LOD0 顶点采样、8 组 fixture 和 Stage 1 门禁；法线重建及 Shadow/Depth pass 同步明确留作后续 |
| #60 | 顶点位移只有静态高度，缺少可复现的波浪、根部锚定风摆和固定时间回归 | 2026-08-26 | 已新增显式时间行进正弦、高度渐变权重、法线波浪/风向摆动组合、15 组 fixture 和 Stage 1 门禁；对象空间连续性及 Shadow/Depth 同步明确留作后续 |
| #61 | BasePass 仍手工编排高度、波浪和风摆低层函数，消费边界与效果顺序容易分叉 | 2026-08-26 | 已新增结构化 `TA_VertexDeformation` 高层模块、单一求值入口、诊断结果、6 组组合 fixture 和 Stage 1 门禁；BasePass 仅保留采样与参数绑定 |
| #67 | 材质源码模块已有分散验收，但缺少同场景综合展板、模块输入输出说明和截图留痕规则 | 2026-09-07 | 已新增六展台 Editor 生成器、独立材质、源码库 README、场景契约、1600×900 离线参考图和专项门禁；真实运行截图仍需有效 Unity Editor 许可证 |
| #68 | HLSL 源码库缺少统一警告清零、空工程迁移、可重复发布包和 v1.0 验收记录 | 2026-09-07 | 已聚合 18 个验证器和 116 项结构检查，迁移 19 个 HLSL 文件并校验 SHA/include 闭包，以 `fxc /WX /Ges` 零警告编译，生成确定性 ZIP、校验和及专项报告 |
