...
Unity 资产导入阶段位于 `UnityMaterialLab/`，固定 Unity `2022.3.62f3c1`、URP `14.0.12`，包含 CC0 OBJ/PNG、统一命名/目录、URP Lit 材质球、BaseColor/Normal/ORM 输入、三个材质实例、11 组参数边界、Prefab、测试场景、导入 Bootstrap、三档 LOD 基础场景、RenderDoc 截帧准备、BasePass 光照拆解、Renderer 侧 HLSL 源码库、六模块综合展板和静态验收。工程步骤见 [Unity 工程与资产导入](UnityMaterialLab/README.md)，输入契约见 [Unity 材质输入与实例](docs/UNITY_MATERIAL_INPUTS.md)，边界结论见 [Unity 材质参数边界验证](docs/UNITY_MATERIAL_BOUNDARIES.md)，Custom Function 接入见 [Shader Graph Custom Function 节点](docs/UNITY_SHADER_GRAPH_CUSTOM_FUNCTION.md)，函数库见 [Unity 材质函数库](docs/UNITY_MATERIAL_FUNCTION_LIBRARY.md)，源码库见 [Unity HLSL 源码库骨架](docs/UNITY_HLSL_SOURCE_LIBRARY.md)，v1.0 发布验收见 [Unity HLSL 专项验收](docs/UNITY_HLSL_SPECIAL_ACCEPTANCE.md)，向量与采样接口见 [Unity 向量与采样工具函数](docs/UNITY_VECTOR_SAMPLING_UTILITIES.md)，简化 PBR 输入见 [Unity 简化 PBR 输入层](docs/UNITY_SIMPLIFIED_PBR_INPUT_LAYER.md)，GGX NDF 见 [Unity GGX 法线分布](docs/UNITY_GGX_NORMAL_DISTRIBUTION.md)，几何遮蔽与 Fresnel 见 [Unity 几何遮蔽与 Fresnel](docs/UNITY_GGX_GEOMETRY_FRESNEL.md)，各向异性基础见 [Unity 各向异性基础](docs/UNITY_ANISOTROPY_BASICS.md)，整合见 [Unity 各向异性 PBR 整合](docs/UNITY_ANISOTROPIC_PBR_INTEGRATION.md)，顶点位移见 [Unity 顶点位移基础](docs/UNITY_VERTEX_DISPLACEMENT_BASICS.md)，波浪风摆见 [Unity 波浪与风摆动画](docs/UNITY_WAVE_WIND_ANIMATION.md)，位移模块化见 [Unity 顶点位移模块化](docs/UNITY_VERTEX_DISPLACEMENT_MODULARIZATION.md)，压缩策略见 [Unity 贴图压缩](docs/UNITY_TEXTURE_COMPRESSION.md)，LOD 规则见 [Unity LOD 基础](docs/UNITY_LOD_BASICS.md)，截帧准备见 [Unity RenderDoc 截帧准备](docs/UNITY_RENDERDOC_CAPTURE_PREPARATION.md)，拆解视图见 [Unity BasePass 与光照拆解](docs/UNITY_BASEPASS_LIGHTING_DECOMPOSITION.md)，综合示例见 [Unity 示例场景与文档](docs/UNITY_MATERIAL_SHOWCASE.md)。
...
Renderer 侧共享 HLSL 已拆分为 18 个模块，并由 `TA_ShaderLibrary.hlsl` 稳定聚合。`TA_VertexDeformation.hlsl` 统一高度、波浪、风摆顺序，程序化遮罩、边缘磨损和积雪模块组合表面响应，`TA_Anisotropy.hlsl` 通过统一镜面项入口把旋转 T/B 基和各向异性 GGX 接入直接光 PBR。模块边界见 [Unity 各向异性基础](docs/UNITY_ANISOTROPY_BASICS.md) 和 [Unity 各向异性 PBR 整合](docs/UNITY_ANISOTROPIC_PBR_INTEGRATION.md)。

HLSL 材质源码库 v1.0 发布包位于 `UnityMaterialLab/Releases/TA_HLSL_MaterialLibrary_v1.0.0.zip`。专项门禁聚合 18 个验证器和 116 项源码结构检查，从两文件空 Unity 工程骨架迁移并校验哈希/include 闭包，再以 Windows SDK `fxc /WX /Ges` 对全部模块执行零警告编译；复现命令和限制见 [Unity HLSL 专项验收](docs/UNITY_HLSL_SPECIAL_ACCEPTANCE.md)。

执行 Unity 菜单 `TA/Material Lab/Build Material Showcase` 可生成六模块同场景材质展板，覆盖基础 PBR、多层法线、边缘磨损、积雪、各向异性金属和透明折射。无 Editor 许可证时运行 `UnityMaterialLab/Tools/ValidateMaterialShowcase.ps1` 生成标注为离线参考的 `MaterialShowcaseReference.png`；完整步骤、输入输出和失败点见 [Unity 示例场景与文档](docs/UNITY_MATERIAL_SHOWCASE.md)。
...
Unity BasePass 现支持高度图 LOD0、中心值和振幅驱动的对象空间法线位移，默认零振幅保持已有材质基线。8 组专项回归及 Forward-only 边界见 [Unity 顶点位移基础](docs/UNITY_VERTEX_DISPLACEMENT_BASICS.md)。

## 波浪与风摆动画

Unity BasePass 现使用显式时间的行进正弦生成法线波浪与高度锚定风摆；两种振幅默认均为 `0`。15 组固定时间回归和对象空间边界见 [Unity 波浪与风摆动画](docs/UNITY_WAVE_WIND_ANIMATION.md)。

## 顶点位移模块化

高度位移、波浪和风摆现由 `TA_EvaluateVertexDeformationOS` 单入口统一编排，BasePass 只保留资源采样与参数装配。6 组组合回归和模块依赖边界见 [Unity 顶点位移模块化](docs/UNITY_VERTEX_DISPLACEMENT_MODULARIZATION.md)。
