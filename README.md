...
Renderer 侧共享 HLSL 已拆分为 16 个模块，并由 `TA_ShaderLibrary.hlsl` 稳定聚合。`TA_VertexDeformation.hlsl` 统一高度、波浪、风摆顺序，程序化遮罩、边缘磨损和积雪模块组合表面响应，`TA_Anisotropy.hlsl` 提供旋转 T/B 基与各向异性 GGX 分布/可见性。模块边界见 [Unity 顶点位移模块化](docs/UNITY_VERTEX_DISPLACEMENT_MODULARIZATION.md)、[Unity 程序化遮罩](docs/UNITY_PROCEDURAL_MASK.md)、[Unity 边缘磨损](docs/UNITY_EDGE_WEAR.md)、[Unity 积雪覆盖](docs/UNITY_SNOW_COVER.md) 和 [Unity 各向异性基础](docs/UNITY_ANISOTROPY_BASICS.md)。
