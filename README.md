...
Renderer 侧共享 HLSL 已拆分为 Types、Common、Vector、Sampling、BRDF、Lighting、DebugViews 七个模块，并由 `TA_ShaderLibrary.hlsl` 稳定聚合。BasePass 已改为通过统一 UV、纹理、法线、ORM、TBN 和结构化光照接口调用源码库；模块边界见 [Unity HLSL 源码库骨架](docs/UNITY_HLSL_SOURCE_LIBRARY.md)，工具约定见 [Unity 向量与采样工具函数](docs/UNITY_VECTOR_SAMPLING_UTILITIES.md)。
