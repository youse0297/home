...
VecMath 是一个使用 C++17 编写的轻量级向量与矩阵数学项目，提供二维、三维和四维向量运算、4×4 矩阵运算、左右手坐标系支持，以及图形学中常用的平移、缩放、旋转变换、相机视图矩阵和透视投影矩阵。

项目当前以控制台程序演示透视投影矩阵以及视锥体关键点到 NDC 的映射结果，不依赖第三方数学库。
...
```powershell
g++ -std=gnu++17 -finput-charset=UTF-8 -fexec-charset=UTF-8 -g src/main.cpp src/Vec2.cpp src/Vec3.cpp src/VectorUtils.cpp src/Vec4.cpp src/Mat4.cpp src/Transform.cpp src/Camera.cpp src/Projection.cpp -o main.exe
./main.exe
```
...
- 透视除法需要在投影变换后由调用方执行，即将裁剪空间坐标除以 `w` 得到 NDC 坐标。
