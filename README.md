...
Renderer 侧共享 HLSL 已拆分为 16 个模块，并由 `TA_ShaderLibrary.hlsl` 稳定聚合。`TA_VertexDeformation.hlsl` 统一高度、波浪、风摆顺序，程序化遮罩、边缘磨损和积雪模块组合表面响应，`TA_Anisotropy.hlsl` 通过统一镜面项入口把旋转 T/B 基和各向异性 GGX 接入直接光 PBR。模块边界见 [Unity 各向异性基础](docs/UNITY_ANISOTROPY_BASICS.md) 和 [Unity 各向异性 PBR 整合](docs/UNITY_ANISOTROPIC_PBR_INTEGRATION.md)。
