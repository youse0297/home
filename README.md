...
Renderer 侧共享 HLSL 已拆分为 Types、Common、BRDF、Lighting、DebugViews 五个模块，并由 `TA_ShaderLibrary.hlsl` 稳定聚合。BasePass 已改为通过结构化输入输出调用源码库；模块边界、公共符号与专项验收见 [Unity HLSL 源码库骨架](docs/UNITY_HLSL_SOURCE_LIBRARY.md)。
