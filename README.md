...
Renderer 侧共享 HLSL 已拆分为 15 个模块，并由 `TA_ShaderLibrary.hlsl` 稳定聚合。`TA_VertexDeformation.hlsl` 通过结构化输入、配置、结果和单一入口统一高度、波浪、风摆顺序，`TA_ProceduralMask.hlsl` 提供显式时间驱动的 UV 程序化遮罩，`TA_EdgeWear.hlsl` 提供视角掠射边缘磨损，`TA_SnowCover.hlsl` 提供世界向上积雪覆盖。模块边界见 [Unity 顶点位移模块化](docs/UNITY_VERTEX_DISPLACEMENT_MODULARIZATION.md)、[Unity 程序化遮罩](docs/UNITY_PROCEDURAL_MASK.md)、[Unity 边缘磨损](docs/UNITY_EDGE_WEAR.md) 和 [Unity 积雪覆盖](docs/UNITY_SNOW_COVER.md)。
